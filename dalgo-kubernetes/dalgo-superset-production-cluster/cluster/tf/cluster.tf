...
data "aws_s3_bucket" "terraform_state" {
  bucket = "production-superset-eks-tf-state"
}

# DynamoDB for State Locking - ensure it exists
data "aws_dynamodb_table" "terraform_locks" {
  name         = "production-superset-eks-tf-state-locks"
}
...
terraform {
  backend "s3" {
    ...
    region         = "ap-south-1"
    dynamodb_table = "production-superset-eks-tf-state-locks"
    encrypt        = true
  }
}
...
resource "aws_eks_cluster" "eks_cluster" {
  name     = "dalgo-superset-production-cluster"
...
resource "aws_security_group" "eks_cluster_sg" {
  name        = "dalgo-superset-production-cluster-sg"
...
resource "aws_security_group" "eks_nodes_sg" {
  name        = "dalgo-superset-production-nodes-sg"
  description = "Security group for worker nodes"
  vpc_id      = data.aws_vpc.current_vpc.id

  tags = {
    Name                                          = "dalgo-superset-production-nodes-sg"
    "kubernetes.io/cluster/dalgo-superset-production-cluster"    = "owned"
  }
}
...
resource "aws_iam_role" "eks_cluster_autoscaler_role" {
  ...
  name               = "eks-superset-cluster-autoscaler-production"
}
...
resource "aws_eks_node_group" "eks_node_group" {
  ...
  node_group_name = "eks-superset-production-node-group"
  ...
  tags = {
    ...
    "k8s.io/cluster-autoscaler/dalgo-superset-production-cluster" = "owned"
  }
      ...
      spec {
        ...
        container {
          ...
          command = [
            ...
            "--node-group-auto-discovery=asg:tag=k8s.io/cluster-autoscaler/enabled,k8s.io/cluster-autoscaler/dalgo-superset-production-cluster"
          ]
